...
if ($_POST["donow"] == "checkForm") {
	...
	if ($protectbuild == false) {
		...
		if(move_uploaded_file($_FILES['startup']['tmp_name'], "../startup_image.png")) {
			$uploadedsomething ="true";
			$message .= "<li>Startpage has been uploaded!</li>";
		} else {
			if (file_exists ("../startup_image.png")) {
				} else {
					$message .= "<li>No startpage uploaded yet</li>";
				} 
		} 
	}
	...
	#check dimensions of start up image
	if (file_exists ("../startup_image.png")) {
		list($width, $height, $type, $attr) = getimagesize("../startup_image.png");
		if ($width != $targetwidth || $height != $targetheight-$statusbarheight){
			$message .= "<li class=\"warning\">Check the size of your start up image. It should be ".$targetwidth." by ". ($targetheight-$statusbarheight) ."pixels</li>";
			}
	}
...
body {
	margin: 0px;
	background-image: url(http://unitid.nl/wp-content/themes/unitid/images/back_paper.gif);
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
	  ...
	  if ($("#other").is(":checked"))
	   ...
	   $("#ipad, #ipadretina #iphone4, #iphone, #other").click(function(){
		if ($("#other").is(":checked"))
        {
            $(".otherfields").show();
        }
      ...
      <table cellpadding="3" cellspacing="0">
        ...
        <tr  class="appfields">
          ...
          <td><select name="deviceframe" id="deviceframe">
              ...
              <option value="iphone3" <?php if (!(strcmp("iphone3", "$deviceframe"))) {echo "selected=\"selected\"";} ?>>iPhone 3</option>
              <option value="iphone4" <?php if (!(strcmp("iphone4", "$deviceframe"))) {echo "selected=\"selected\"";} ?>>iPhone 4</option>
              <option value="torch9800" <?php if (!(strcmp("torch9800", "$deviceframe"))) {echo "selected=\"selected\"";} ?>>Blackberry Torch 9800 (experimental)</option>
            </select></td>
        ...
        <tr>
          ...
          <td class="infotext">Color outside of the prototype. Visible in browsers larger than the prototype and seen when using transistions like &quot;flip&quot;. Default: #0000000 (black). Use #FFFFFF for white.</td>
        </tr>
        <tr  class="appfields">
          <td>&nbsp;</td>
          <td>&nbsp;</td>
        ...
        <tr  class="appfields">
          ...
          <td class="infotext">Screen that appears while loading the app. For iPhone: 320 by 460 pixels. For iPad 768 by 1004 pixels. If you want this in Retina, multiply by 2
            <?php if (file_exists ("../startup_image.png")) {?>
            <br/>
            <img src="../startup_image.png" height="80px" width="60px" />
            <?php } ?></td>
        </tr>
        <tr>
